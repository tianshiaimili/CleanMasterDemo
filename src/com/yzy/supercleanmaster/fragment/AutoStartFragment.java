...
public class AutoStartFragment extends BaseFragment {
	...
	private void fillData() {
		...
		List<AutoStartInfo> mAutoStartInfo = BootStartUtils
				.fetchAutoApps(mContext);

		// List<AutoStartInfo> mAutoStartInfo =
		// BootStartUtils.fetchInstalledApps(mContext);
		...
		if (position == 0) {
			mAutoStartAdapter = new AutoStartAdapter(mContext, noSystemAuto,
					mHandler);
			listview.setAdapter(mAutoStartAdapter);
			refeshButoom();
		} else {

			mAutoStartAdapter = new AutoStartAdapter(mContext, isSystemAuto,
					null);
			listview.setAdapter(mAutoStartAdapter);
